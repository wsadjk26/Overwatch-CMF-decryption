...
{
  uint32_t kidx = m_keytable[(length * m_keytable[0]) % 512];
...
void GenerateIV(uint8_t* buffer, const CMFHeader* header, const std::string& name, const int32_t length)
{
  ...
  uint32_t kidx = m_keytable[(digest[7] * m_keytable[0]) & 511];
  uint32_t increment = kidx % 29;
  for (int32_t i = 0; i != length; ++i)
  {
    buffer[i] = m_keytable[kidx % 512];
    kidx += increment;
    buffer[i] ^= digest[(kidx + header->m_entriesCount) % SHA1::DIGESTSIZE] + 1;
  }
}
