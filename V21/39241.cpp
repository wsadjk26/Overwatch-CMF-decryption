...
struct CMFHeader
{
  ...
  // 0x15666D63 '\x15fmc' -> Not Encrypted
  // 0x636D6615 'cmf\x15' -> Encrypted
  uint32_t m_magicbytes;
};

void GenerateKey(uint8_t* buffer, const CMFHeader* header, const int32_t length)
{
  uint32_t kidx = m_keytable[header->m_dataCount & 511];
  uint32_t increment = kidx % 61;
...
void GenerateIV(uint8_t* buffer, const CMFHeader* header, const std::string& name, const int32_t length)
{
  ...
  uint32_t kidx = (header->m_dataCount + digest[7]) & 511;
  uint32_t increment = header->m_buildVersion * header->m_dataCount % 7;
  for (int32_t i = 0; i != length; ++i)
  {
    buffer[i] = m_keytable[kidx % 512];
    kidx += increment;
    buffer[i] ^= digest[(kidx - 73) % SHA1::DIGESTSIZE];
  }
}
